...
from lt_eli_mcp.coverage import build_coverage
from lt_eli_mcp.server import lt_coverage


def _coverage():
    fn = getattr(lt_coverage, "fn", lt_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LT_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("lt_coverage" in line for line in lines), (
        "an audit entry was written but it does not name lt_coverage"
    )
