...
SOURCE = 'get.data.gov.lt (TAR via the Spinta API, CC BY 4.0)'
...
_FAMILIES: list[dict] = [{'name': 'Register of Legal Acts (TAR)', 'tool': 'lt_search / lt_get_act / lt_get_text'}]

_GAPS: list[dict] = [{'id': 'LT-001', 'family': 'Register of Legal Acts (TAR)', 'missing': 'Lithuania has no data.europa.eu ELI for the TAR dataset; eli_uri carries the canonical e-tar.lt URL instead.', 'fallback': 'Treat eli_uri as the national e-tar.lt identifier.'}, {'id': 'LT-002', 'family': 'Register of Legal Acts (TAR)', 'missing': 'Case law is not covered - TAR is a legislation register.', 'fallback': "Use the Lithuanian courts' own decision search."}, {'id': 'LT-003', 'family': 'Register of Legal Acts (TAR)', 'missing': 'Legal force on a given date is not evaluated. Text is returned as the source publishes it; this connector does not decide whether a provision is in force.', 'fallback': 'Check the validity or consolidation dates on the source record itself.'}]
