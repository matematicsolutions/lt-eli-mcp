...
Pulls a tiny static ``lt-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/lt-runtime.json
...
Opt-out: set ``LT_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "LT_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/lt-eli-mcp/releases/latest/download/lt-runtime.json.gz",
)
_CACHE_NAME = "lt-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "lt-eli-mcp-runtime (+https://github.com/matematicsolutions/lt-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("LT_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "lt-eli"
